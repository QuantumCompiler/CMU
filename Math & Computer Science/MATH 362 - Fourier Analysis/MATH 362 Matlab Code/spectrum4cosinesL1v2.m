...
f(1:M) = cos(2*pi*freq1*t(1:M));
f(M+1:2*M) = cos(2*pi*freq2*t(M+1:2*M));
f(2*M+1:3*M) = cos(2*pi*freq3*t(2*M+1:3*M));
f(3*M+1:4*M) = cos(2*pi*freq4*t(3*M+1:4*M));
...
c2 = 1/M*fft(f(M+1:2*M));
c2mag = abs(c2(1:right))';

c3 = 1/M*fft(f(2*M+1:3*M));
c3mag = abs(c3(1:right))';

c4 = 1/M*fft(f(3*M+1:4*M));
